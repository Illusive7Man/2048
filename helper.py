...
class PlayerAI(BaseAI):
    ...
    def maximize(self, state, a, b, depth=0):
        if time.clock() - self.start > 0.099:
    ...
    def minimize(self, state, a, b, depth):
        ...
        for position in available:
            children.append(state.clone())
            children[-1].setCellValue(position, 2)
            if len(children) > 5:
                break
    ...
    def evaluate(self, state):
        available = state.getAvailableCells()
        return (10 * self.max_tile_pos(state, available) + 5 * self.gradient(state) + self.smoothness(state)
                + 10 * len(state.getAvailableCells()))
    ...
    def getMove(self, grid):
        ...
        while True:
            state1, rest = self.maximize(grid, -math.inf, math.inf)
        ...
        try:
            for i in self.preferred_order:
                state1 = grid.clone()
                state1.move(i)
                if state.map == state1.map:
                    move = i
                    break
        except AttributeError:
            for i in self.preferred_order:
                if i in grid.getAvailableMoves():
                    move = i
                    break
